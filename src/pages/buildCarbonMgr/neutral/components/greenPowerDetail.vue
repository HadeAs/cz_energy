...
<template>
  ...
  >
    ...
    <el-form-item label="绿色家电购买抵消(t)" required prop="greenPowerAmount">
      <el-input-number placeholder="请输入绿色家电购买抵消" v-model="state.detailForm.greenPowerAmount" />
    </el-form-item>
  </el-form>
</template>
<script setup>
...
const rules = {
  ...
  greenPowerAmount: { required: true, message: "请输入绿色家电购买抵消", trigger: "change" },
};
...
const loadData = async () => {
  const res = await getConfig({ projectId: globalState.value.projectId, year: state.detailForm.year });
  console.log(`res`, res);
